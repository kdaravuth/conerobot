...
public class Readbrasconvertbbuserid {

	public static void formatBrasFile(String siteToRun, String fileName) throws IOException, ClassNotFoundException, SQLException {

		//Database connection
				RobotUtilities r = new RobotUtilities();
				
				Connection con = r.dbconnection("oracle.jdbc.driver.OracleDriver", "jdbc:oracle:thin:@10.6.1.203:1521:brs", "brssystem", "btcchamp");
				...
				PreparedStatement psPCRF = conPCRF.prepareStatement("insert into BTC_PCRF.BRAS_CONVERTED_OUTPUT values(?,?,?,?,?,?,?)");
		...
		try {
			...
			while ((line = bfrdr.readLine()) != null) {
				...
				if (findIFStartingPoint(line.toString(), "user-vlan")) {
					tempSubs = tempSubs + "::" + line.toString().replaceAll("[\\t\\n\\r]+", " ");
				}

				// access-type layer2-subscriber default-domain pre-authentication

				if (findIFStartingPoint(line.toString(),
						"access-type layer2-subscriber default-domain pre-authentication")) {
					tempSubs = tempSubs + "::" + line.toString().replaceAll("[\\t\\n\\r]+", " ");
			...
			FileWriter output = new FileWriter("src/output/" + siteToRun + "_OP_" + ft.format(date) + ".out", true);
			...
			for (int i = 0; i < brasSub.size(); i++) {
				...
				// no need to show
				if (brasSub.get(i).split("::").length >= 5) {
					if (brasSub.get(i).split("::")[1].replaceAll("\\D", "").length() > 10) {
										
						System.out.println(
								"BTCBANDWIDTH::" + getCleanExternalID(brasSub.get(i).split("::")[1].substring(getSubstrloc(brasSub.get(i).split("::")[1],"description")))
										+ "::" + convertBBUserID(siteToRun, brasSub.get(i).toString()) + "::"
										+ brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1] + "::"
										+ brasSub.get(i).split("::")[0].replaceAll(" ", "") + "::"
										+ brasSub.get(i).split("::")[1].replaceAll("[0-9]", "")
												.replace("description", "").replace("*", "").replace("  ", " ")
												.replace("---- More ----[D                     [D", "")); // +
						// brasSub.get(i).split("::")[4].replaceAll("\\D", ""));
						String tout = "BTCBANDWIDTH::"
								+ getCleanExternalID(brasSub.get(i).split("::")[1].substring(getSubstrloc(brasSub.get(i).split("::")[1],"description"))) + "::"
								+ convertBBUserID(siteToRun, brasSub.get(i).toString()) + "::"
								+ brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1] + "::"
								+ brasSub.get(i).split("::")[0].replaceAll(" ", "") + "::"
						...
						//PreparedStatement ps = con.prepareStatement("insert into BRAS_CONVERTED_OUTPUT values(?,?,?,?,?,?,?)");
						ps.setString(1,"BTCBANDWIDTH");
						ps.setString(2, getCleanExternalID(brasSub.get(i).split("::")[1].substring(getSubstrloc(brasSub.get(i).split("::")[1],"description"))));
						ps.setString(3, convertBBUserID(siteToRun, brasSub.get(i).toString()));
						ps.setString(4, brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1]);
						ps.setString(5, brasSub.get(i).split("::")[0].replaceAll(" ", ""));
						ps.setString(6, brasSub.get(i).split("::")[1].replaceAll("[0-9]", "").replace("description", "")
								.replace("*", "").replace("  ", " ")
								.replace("---- More ----[D                     [D", ""));
						...
						psPCRF.setString(1,"BTCBANDWIDTH");
						psPCRF.setString(2, getCleanExternalID(brasSub.get(i).split("::")[1].substring(getSubstrloc(brasSub.get(i).split("::")[1],"description"))));
						psPCRF.setString(3, convertBBUserID(siteToRun, brasSub.get(i).toString()));
						psPCRF.setString(4, brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1]);
						psPCRF.setString(5, brasSub.get(i).split("::")[0].replaceAll(" ", ""));
						psPCRF.setString(6, brasSub.get(i).split("::")[1].replaceAll("[0-9]", "").replace("description", "")
								.replace("*", "").replace("  ", " ")
								.replace("---- More ----[D                     [D", ""));
						...
						System.out
								.println("BTCBANDWIDTH::" + getCleanExternalID(brasSub.get(i).split("::")[1].substring(getSubstrloc(brasSub.get(i).split("::")[1],"description"))) + "::"
										+ convertBBUserID(siteToRun, brasSub.get(i).toString()) + "::"
										+ brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1] + "::"
						...
						String tout = "BTCBANDWIDTH::" + getCleanExternalID(brasSub.get(i).split("::")[1].substring(getSubstrloc(brasSub.get(i).split("::")[1],"description"))) + "::"
								+ convertBBUserID(siteToRun, brasSub.get(i).toString()) + "::"
								+ brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1] + "::"
								+ brasSub.get(i).split("::")[0].replace(
										"---- More ----[42D                                          [42", "")
						...
						ps.setString(3, convertBBUserID(siteToRun, brasSub.get(i).toString()));
						ps.setString(4, brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1]);
						ps.setString(5, brasSub.get(i).split("::")[0].replace(
								"---- More ----[42D                                          [42", ""));
						ps.setString(6, brasSub.get(i).split("::")[1].replaceAll("[0-9]", "").replace("description", "")
								.replace("*", "").replace("  ", " ")
								.replace("---- More ----[D                     [D", ""));
						...
						psPCRF.setString(3, convertBBUserID(siteToRun, brasSub.get(i).toString()));
						psPCRF.setString(4, brasSub.get(i).split("::")[4].substring(getSubstrloc(brasSub.get(i).split("::")[4], "qos-profile")).split(" ")[1]);
						psPCRF.setString(5, brasSub.get(i).split("::")[0].replace(
								"---- More ----[42D                                          [42", ""));
						psPCRF.setString(6, brasSub.get(i).split("::")[1].replaceAll("[0-9]", "").replace("description", "")
								.replace("*", "").replace("  ", " ")
								.replace("---- More ----[D                     [D", ""));
