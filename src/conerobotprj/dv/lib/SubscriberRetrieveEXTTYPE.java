 ...
 *
 */
public class SubscriberRetrieveEXTTYPE {
	...
	static {
		...
		try {
			...
			InputStream configFile = SubscriberRetrieveEXTTYPE.class.getResourceAsStream("/config/logger.cfg");
			LogManager.getLogManager().readConfiguration(configFile);
			LOGGER = Logger.getLogger(SubscriberRetrieveEXTTYPE.class.getName());
		} catch (IOException e) {
			e.getMessage();
		}
	...
	private static void createSoapEnvelope(SOAPMessage soapMessage, String MSISDN, String ExternalIDTYPE)
		...
		c1soapBodysubscriberExternalIdTypeValue.addTextNode(ExternalIDTYPE);

		SOAPElement c1soapBodyinfo = c1soapBodyInput.addChildElement("info");
		SOAPElement attribs = c1soapBodyinfo.addChildElement("attribs");
		...
		SOAPElement balances = c1soapBodyinfo.addChildElement("balances");
		balances.addAttribute(setQname, "true");
		balances.addAttribute(changedQname, "true");
		
		SOAPElement balancesValue = balances.addChildElement("value");
		balancesValue.addTextNode("true");

	}

	// Create SOAP request
	private static SOAPMessage createSOAPRequest(String soapAction, String MSISDN, String ExternalIDTYPE)
			throws Exception {
		MessageFactory messageFactory = MessageFactory.newInstance();
		SOAPMessage soapMessage = messageFactory.createMessage();

		createSoapEnvelope(soapMessage, MSISDN, ExternalIDTYPE);
...
// Call Subscriber retrieve soap:
	public void callSubscriberRetrieveService(String MSISDN, String ExternalIDTYPE) {
		...
		try {
			...
			SOAPMessage soapResponse = soapConnection.call(createSOAPRequest(soapAction, MSISDN, ExternalIDTYPE),
					soapEndpointUrl);
