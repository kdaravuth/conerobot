...
public class OrderCancelCSM {
	...
	public static void createSECancelOrder(SOAPMessage soapMessage, String CancelOrder)
		...
		SOAPElement OrderCancel = c1soapBody.addChildElement("OrderCancel", myNamespace);

		SOAPElement SAOIInput = OrderCancel.addChildElement("input", myNamespace);
		...
		orderIdValue.addTextNode(CancelOrder);

	}
	// End create offer add envelope

	// Create SOAP request
	private static SOAPMessage createSOAPRequestCancelOrder(String soapAction, String CancelOrder)
			throws Exception {
		MessageFactory messageFactory = MessageFactory.newInstance();
		SOAPMessage soapMessage = messageFactory.createMessage();

		createSECancelOrder(soapMessage, CancelOrder);
	...
	public static void callingCancelOrder(String CancelOrder)
		...
		try {
			...
			LOGGER.log(Level.INFO, "Calling Order Cancelling...");
			SOAPMessage soapResponse = soapConnection.call(createSOAPRequestCancelOrder(soapAction, CancelOrder),
					soapEndpointUrl);
			...
			soapResponse.writeTo(new FileOutputStream(new File("src/input/CancelOrderResponse.xml")));
			soapConnection.close();

			// Read Subscriber retrieve response from temp xml file
			LOGGER.log(Level.INFO, "Simplified Results --> " + CancelOrder);

			File xmlresponse = new File("src/input/CancelOrderResponse.xml");
			...
			if (childnodes.item(0).getNodeName().equals("S:Body")) {
					...
					if (child.getNodeType() == Node.ELEMENT_NODE) {
						if (childnodes.item(i).getTextContent().trim() != "") {
							...
							LOGGER.log(Level.SEVERE,
									"RESULT FAIL " + CancelOrder + "::" + "::" + childnodes.item(i).getNodeName() + "::"
				...
				LOGGER.log(Level.INFO, "RESULT SUCCESS : " + CancelOrder + "::" + tempSubInfo);

			}

			LOGGER.log(Level.INFO, "<-- End Simplified result: " + CancelOrder);
