...
public class OfferAddExtended {
	...
	public static String puborder = "init";
	public static String pubserviceorder = "init";
	...
	public static void createSEOfferAdd(SOAPMessage soapMessage, String MSISDN, String OfferID,String optOfferId, String OrderId, String ServiceOrderId)
		...
		SOAPElement newOptionalOfferIdList = SAOIInput.addChildElement("newSupplementaryOfferIdList");
		SOAPElement optionalOfferIid = newOptionalOfferIdList.addChildElement("offerId");
		optionalOfferIid.addAttribute(setQname, "true");
		optionalOfferIid.addAttribute(changedQname, "true");
		SOAPElement optionalOfferIidValue = optionalOfferIid.addChildElement("value");
		optionalOfferIidValue.addTextNode(optOfferId);
		
		if (!ServiceOrderId.equals("NA") && !OrderId.equals("NA")) {
			
			SOAPElement serviceOrderId = SAOIInput.addChildElement("serviceOrderId");
			
			SOAPElement serviceOrderIdsub = serviceOrderId.addChildElement("serviceOrderId");
			serviceOrderIdsub.addAttribute(setQname, "true");
			serviceOrderIdsub.addAttribute(changedQname, "true");
			SOAPElement serviceOrderIdValue = serviceOrderIdsub.addChildElement("value");
			serviceOrderIdValue.addTextNode(ServiceOrderId);
			
			SOAPElement orderId = serviceOrderId.addChildElement("orderId");
			...
			orderIdValue.addTextNode(OrderId);
		...
		orderAutoCommit.addTextNode("1");

		SOAPElement generateWorkflow = SAOIInput.addChildElement("generateWorkflow");
		generateWorkflow.addTextNode("1");

	}
	// End create offer add envelope

	// Create SOAP request
	private static SOAPMessage createSOAPRequest(String soapAction, String MSISDN, String OfferID,String optOfferId, String OrderId, String ServiceOrderId)
			throws Exception {
		MessageFactory messageFactory = MessageFactory.newInstance();
		SOAPMessage soapMessage = messageFactory.createMessage();

		createSEOfferAdd(soapMessage,MSISDN, OfferID,optOfferId, OrderId, ServiceOrderId);
	...
	public static void callingOfferAdd(String MSISDN, String OfferID, String optOfferId, String OrderId, String ServiceOrderId)
		...
		try {
			...
			SOAPMessage soapResponse = soapConnection.call(createSOAPRequest(soapAction, MSISDN, OfferID,optOfferId, OrderId, ServiceOrderId),
					soapEndpointUrl);
			...
			if (childnodes.item(0).getNodeName().equals("S:Body")) {
					...
					if (child.getNodeType() == Node.ELEMENT_NODE) {
						if (childnodes.item(i).getTextContent().trim() != "") {
							...
							LOGGER.log(Level.SEVERE,
									"RESULT FAIL::"+OfferID+"++" + MSISDN+ ";"+tsubscrno+";" +tsubscrnoreset+ "::" + childnodes.item(i).getNodeName() + "::"
				...
				for (int i = 0; i < childnodesSubInfo.getLength(); i++) {
					...
					if (child.getNodeType() == Node.ELEMENT_NODE) {
						if (child.getNodeName().equals("orderIdentifier")) {
							...
							puborder = child.getChildNodes().item(2).getTextContent(); 
							
							LOGGER.log(Level.INFO, "OrderID: " + puborder );
						}
					if (child.getNodeName().equals("serviceOrderId")) {
							
							pubserviceorder = child.getChildNodes().item(1).getTextContent(); 
							
							LOGGER.log(Level.INFO, "Service Order ID: " + pubserviceorder );
						}
				...
				LOGGER.log(Level.INFO, "RESULT SUCCESS::" + MSISDN+ ";"+tsubscrno+";" +tsubscrnoreset+ "::" + tempSubInfo);
