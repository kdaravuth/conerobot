...
public class DirectoryServiceSubsCreation {
	// Initialize logger
	...
	public static String offerInstId = "";
	public static String rcTermInstId = "";
	public static int errorFindRCTerm = 0;
	public static int errorCreatingSub = 0;
	...
	public static void createSESCFO(SOAPMessage soapMessage, String ExternalID, String ExternalIDType, String Company,
			...
			String ActiveDT, String CITY, String STATE, String POSTALCODE, String COUNTRYCODE) throws SOAPException {
		...
		// String subExtendedData = "<![CDATA[<ExtendedData><Parameter
		// name=\"Nationality\"><IntegerValue>72</IntegerValue></Parameter><Parameter
		// name=\"Customer
		// Profession\"><StringValue>Government</StringValue></Parameter><Parameter
		// name=\"notif_number\"><StringValue>26773000000</StringValue></Parameter></ExtendedData>]]>";
		// extendedDataValue.addTextNode(CDATA.normalizeString(subExtendedData));

		String subExtendedData = "<ExtendedData><Parameter name=\"Nationality\"><IntegerValue>72</IntegerValue></Parameter><Parameter name=\"Customer Profession\"><StringValue>Government</StringValue></Parameter><Parameter name=\"notif_number\"><StringValue>00000000000</StringValue></Parameter></ExtendedData>";
		extendedDataValue.addTextNode(CDATA.normalizeString(subExtendedData));
		...
		SOAPElement externalIdValue = externalId.addChildElement("value");
		...
		// String extendedValue = "<![CDATA[<ExtendedData><Parameter
		// name=\"original_external_id\"><StringValue>2673123123</StringValue></Parameter><Parameter
		// name=\"annotation\"><StringValue>0</StringValue></Parameter><Parameter\r\n"
		// + "name=\"Fluency\"><IntegerValue>1</IntegerValue></Parameter><Parameter
		// name=\"SYSPRO_INVENTORY_COUNT\"><IntegerValue>1</IntegerValue></Parameter>]]>";

		String extendedValue = "<ExtendedData><Parameter name=\"original_external_id\"><StringValue>2673123123</StringValue></Parameter><Parameter name=\"annotation\"><StringValue>0</StringValue></Parameter><Parameter name=\"Fluency\"><IntegerValue>1</IntegerValue></Parameter><Parameter name=\"SYSPRO_INVENTORY_COUNT\"><IntegerValue>1</IntegerValue></Parameter>";

		offerExtValue.addTextNode(CDATA.normalizeString(extendedValue));
...
//end Add extended data for offer
		...
		state.addAttribute(setQname, "true");
		state.addAttribute(changedQname, "true");
		SOAPElement stateValue = state.addChildElement("value");
		stateValue.addTextNode(STATE);
		...
		postalCodeValue.addTextNode(POSTALCODE);
		...
		countryCodeValue.addTextNode(COUNTRYCODE);

		SOAPElement effectiveDate = input.addChildElement("effectiveDate");
		// effectiveDate.addTextNode("2020-07-27T00:00:00.0");
		effectiveDate.addTextNode(ActiveDT);
	...
	public static SOAPMessage createSRSCFO(String soapAction, String ExternalID, String ExternalIDType, String Company,
			String Fname, String lname, String AcctNo, String address1, String address2, String offerID,
			String ActiveDT, String CITY, String STATE, String POSTALCODE, String COUNTRYCODE) throws Exception {

		MessageFactory messageFactory = MessageFactory.newInstance();
		SOAPMessage soapMessage = messageFactory.createMessage();

		createSESCFO(soapMessage, ExternalID, ExternalIDType, Company, Fname, lname, AcctNo, address1, address2,
				offerID, ActiveDT, CITY, STATE, POSTALCODE, COUNTRYCODE);
	...
	public void callSCFO(String ExternalID, String ExternalIDType, String Company, String Fname, String lname,
			String AcctNo, String address1, String address2, String offerID, String ActiveDT, String CITY, String STATE,
		...
		try {
			...
			SOAPMessage soapResponse = soapConnection.call(createSRSCFO(soapAction, ExternalID, ExternalIDType, Company,
					Fname, lname, AcctNo, address1, address2, offerID, ActiveDT, CITY, STATE, POSTALCODE, COUNTRYCODE),
					soapEndpointUrl);
			...
			LOGGER.log(Level.INFO, "CREATING SUBSCRIBER --> " + ExternalID);
			...
			if (childnodes.item(0).getNodeName().equals("S:Body")) {
				...
				// Get order and offerinstid to override and commit order after everything is
				// good.

				orderId = el.getElementsByTagName("orderId").item(0).getTextContent();
				serviceOrderId = el.getElementsByTagName("serviceOrderId").item(0).getTextContent();
				offerInstId = el.getElementsByTagName("offerInstId").item(0).getTextContent();

				// LOGGER.log(Level.INFO, "ORDERID: " + orderId);
				// LOGGER.log(Level.INFO, "SERVICEORDERID: " + serviceOrderId);
				// LOGGER.log(Level.INFO, "offerInstId: " + offerInstId);

				// Find RCTERMINSTID based on offerInstID
				DirectoryServiceFindRcTermInst DSF = new DirectoryServiceFindRcTermInst();
				DSF.retreiveCred(new File("src/config/soapconnection.cfg"));
				DSF.callFindRCTermInst(offerInstId);
				rcTermInstId = DSF.rcTermInstId;
				errorFindRCTerm = DSF.errorFindRCTerm;
			...
			LOGGER.log(Level.INFO, "<-- End Processing CREATING SUBSCRIBER -->  " + ExternalID);
