...
public class PCRFCheckSubscriber {
	// Initialize logger
	...
	public boolean callPCRFGetUser(String UserIDExternalid, String SearchType) throws Exception {
		...
		try {
			...
			for (int i = 0; i < childnodes.getLength(); i++) {
				...
				if (child.getNodeType() == Node.ELEMENT_NODE) {
					if (childnodes.item(i).getTextContent().trim() != "") {
						...
						if (childnodes.item(i).getNodeName().equals("login-name")){
							
							loginName = childnodes.item(i).getTextContent().trim();
						}
			...
			if (respCode != 200) {
				...
				LOGGER.log(Level.FINEST, "RESULT FAIL: " + UserIDExternalid + "::" + SearchType);
				return false;
			} else if (checkForValidResponse(response).booleanValue()) {
				// System.out.println(" PCRF SDB Provisioning was successful");
				// response = "SucCess:" + response;
				LOGGER.log(Level.FINEST,
						"RESULT SUCCESS: " + UserIDExternalid + "::" + SearchType + " - OUTPUT: " + result);// +
																											// result);
				return true;
			} else {
