...
public class DirectoryServiceFindRcTermInst {
	// Initialize logger
	...
	public static void createSEFindRCTermInst(SOAPMessage soapMessage, String OfferInstId)
		...
		SOAPElement RcTermInstanceRateOverrideCreate = c1soapBody.addChildElement("RcTermInstanceFind",
				myNamespace);
		...
		SOAPElement rcTermInstanceFilter = input.addChildElement("rcTermInstanceFilter");
		rcTermInstanceFilter.addAttribute(fetchQname, "true");
		SOAPElement offerInstId = rcTermInstanceFilter.addChildElement("offerInstId");
		SOAPElement fetch = offerInstId.addChildElement("offerInstId");
		fetch.addTextNode("false");
		SOAPElement filters = offerInstId.addChildElement("filters");
		...
		SOAPElement value = Equals.addChildElement("value");
		value.addTextNode(OfferInstId);	
		

	}// End create Extended Data add envelope

	// Create SOAP request
	public static SOAPMessage createSRFindRCTermInst(String soapAction, String OfferInstId)
			throws Exception {

		MessageFactory messageFactory = MessageFactory.newInstance();
		SOAPMessage soapMessage = messageFactory.createMessage();

		createSEFindRCTermInst(soapMessage, OfferInstId);
	...
	public void callFindRCTermInst(String OfferInstId ) {
		...
		try {
			...
			LOGGER.log(Level.FINEST, "Start Finding RCTERMINSTID Based on: " + OfferInstId);
			SOAPMessage soapResponse = soapConnection.call(createSRFindRCTermInst(soapAction, OfferInstId),
					soapEndpointUrl);
			...
			soapResponse.writeTo(new FileOutputStream(new File("src/input/FindRCTERMINSTResponse.xml")));
			soapConnection.close();

			// Read Subscriber retrieve response from temp xml file
			LOGGER.log(Level.FINEST, "Getting RC TERM INST ID based on OFFER_INST_ID  --> " + OfferInstId);

			File xmlresponse = new File("src/input/FindRCTERMINSTResponse.xml");
			...
			if (childnodes.item(0).getNodeName().equals("S:Body")) {
				...
				rcTermInstId = el.getElementsByTagName("rcTermInstId").item(0).getTextContent();
			...
			LOGGER.log(Level.FINEST, "<-- End Processing Offer InstanceID " + OfferInstId);
